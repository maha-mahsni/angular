...
exports.updateSuggestion = async (req, res) => {
  try {
    ...
    const current = await Suggestion.getById(id);
    const payload = {
      title,
      description: description ?? current?.description ?? "",
      category: category ?? current?.category ?? "",
      status: status ?? current?.status ?? "en attente",
      nbLikes: nbLikes !== undefined ? nbLikes : (current?.nbLikes ?? 0),
    };
    const affectedRows = await Suggestion.update(id, payload);
...
// PATCH mise à jour partielle (ex: nbLikes pour le frontend Angular)
exports.patchSuggestion = async (req, res) => {
  try {
    const { id } = req.params;
    const body = req.body || {};
    const updated = await Suggestion.patch(id, body);
    if (!updated) {
      return res.status(404).json({
        success: false,
        error: "Suggestion non trouvée",
